...
> 이후 Day 15 계획에서 `Access Token = 메모리`, `Refresh Token = HttpOnly cookie`로 전환하기로 확정했다.
...
- REST Docs, 일정표, 작업 허브, 리뷰 문서를 Day 14 결과에 맞게 갱신
...
- 마이페이지 상세 API를 Day 14에서 함께 구현하면 작업 범위가 불필요하게 커진다.
...
- `/api/me` 실패 시 세션 정리 폴백(Fallback)을 적용할 수 있는 기반이 마련되었다.
...
#### 결과
- Day 14 범위의 토큰 만료 UX 핵심 경로 구현이 완료되었다.
...
- Day 14에서 요구한 인증 설계와 테스트 문서가 코드와 같이 움직이도록 맞췄다.
