...
- 반영한 문서:
  ...
  - `docs/dev-log.md`
- 반영 내용:
  - Day 15 secret/env 정리 완료 상태와 local `.env` 기준 반영
...
- Day 15에서 `AuthContext`, `apiClient` refresh queue, 보호 route 회귀를 검증하려면 최소 실행 환경부터 먼저 필요했다.
...
#### 결과
- Day 15 기준 React auth 최소 회귀 테스트 범위가 닫혔다.
...
- 반영한 문서:
  ...
  - `docs/dev-log.md`
- 반영 내용:
  - React auth 회귀 테스트 완료 상태와 Day 15 남은 범위를 JaCoCo만 남도록 정리
...
#### 한 줄 요약
- 메모리 세션 복구, refresh queue, 보호 route 분기를 React 테스트로 고정해 Day 15 auth 회귀 범위를 닫았다.

### JaCoCo 기준선 추가

#### 문제 상황
- backend는 `./gradlew test`와 REST Docs 흐름은 있었지만, 커버리지 리포트를 생성하거나 CI에서 수집하는 기준선이 없었다.
- Day 15 완료 기준에는 JaCoCo 기준선이 포함돼 있어서 최소 report 생성과 CI 업로드 흐름이 필요했다.
...
#### 결과
- Day 15 기준 JaCoCo report 생성과 CI 회수 흐름이 갖춰졌다.
...
- 반영한 문서:
  ...
  - `docs/dev-log.md`
- 반영 내용:
  - Day 15 완료 처리와 JaCoCo 기준선 도입 상태 반영
...
- 반영한 문서:
  - `docs/dev-log.md`
- 반영 내용:
  - Day 15 JaCoCo 기준선 해석을 “도입”에서 “기준선 보정 완료”까지 확장
...
#### 문제 상황
- Day 15 체크리스트에는 `외부 응답 메시지와 내부 로그 메시지 분리`가 남아 있었지만, 실제 공통 예외 레이어와 보안 예외 응답은 JSON 응답만 만들고 운영 로그 기준이 거의 없었다.
...
- Day 15 목표는 응답 계약을 다시 흔드는 것이 아니라, 현재 계약을 유지한 채 추적성을 보강하는 데 있었다.
...
#### 결과
- Day 15 비수동 체크리스트 기준에서 `예외 체계 정리`, `외부 응답 메시지와 내부 로그 메시지 분리`, `React가 처리할 auth 에러 계약 확정`을 닫을 수 있는 상태가 됐다.
- 수동 검증 항목만 별도로 남겼다.
...
- 반영한 문서:
  ...
  - `docs/dev-log.md`
- 반영 내용:
  - Day 15 비수동 항목 완료와 수동 검증 별도 대기 상태 반영
...
#### 한 줄 요약
- 외부 응답 계약은 유지하고 내부 예외 로그만 보강해 Day 15 비수동 체크리스트를 마감 가능한 상태로 만들었다.
...
#### 결과
- Day 15 체크리스트의 수동 검증 항목까지 모두 닫혔다.
...
- 반영한 문서:
  ...
  - `docs/dev-log.md`
- 반영 내용:
  - Day 15 수동 검증 완료와 전체 마감 상태 반영

#### 한 줄 요약
- 브라우저와 `curl` 기준 수동 검증까지 마쳐 Day 15 auth 마감 조건을 실제 동작 기준으로도 확인했다.
