...
- REST Docs, 설계 문서, 일정 문서, 허브 로그를 Day 16 구현 상태와 맞게 동기화
...
- 목록 API 계약, 프런트 동작, REST Docs HTML이 같은 구조로 정렬됐다.
...
#### 결과
- 커뮤니티 상세에서 댓글 읽기/작성/삭제 흐름이 서버 정책과 함께 닫혔다.
...
#### 작업 내용
- `API Specification`, `Screen Specification`, `Database Design`, `Authentication & Authorization Design`, `Project Overview`를 Day 16 최종 구현 상태와 맞췄다.
- `Project Schedule`, `Internal Schedule.internal`, `dev-log.md`와 Day 16 원본 로그를 갱신했다.
- `backend` 전체 `build`와 `frontend` 전체 `test/lint/build`를 다시 실행했다.
- 브라우저에서 커뮤니티, 댓글, 홈, 피드백, 권한 분기 수동 점검을 끝냈다.

#### 결과
- generated REST Docs HTML과 사람이 읽는 설계 문서가 현재 코드와 같은 기준으로 정렬됐다.
