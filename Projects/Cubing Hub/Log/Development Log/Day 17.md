...
- Day 17 안정화 범위 대상 backend/frontend 자동 회귀 검증을 다시 실행
...
#### 결과
- 인증 토큰은 있지만 사용자 컨텍스트가 사라진 경우의 응답이 공통 예외 계약에 더 가깝게 정렬됐다.
- 마이페이지 실패 화면에서 세션 재정리나 재호출 전 재시도 경로가 명확해졌다.
...
- `FeedbackPage`는 제출 중에도 필드가 활성화되어 있었고, 에러 메시지가 남은 상태에서 입력을 바꾸면 stale message가 유지됐다.
...
- 피드백 폼은 제출 중 중복 조작과 stale message 노출이 줄어드는 형태로 정리됐다.
